 ...
 */
void interpolate_on_surface(const Real* xcoords, const Real* states, int found_idx, Real x_target, Real* interpolated_states) {
    ...
    for (int q = 0; q < 4; ++q) {
        Real ys[4] = {
            states[colMajorIndex(q,found_idx-1,4)],
            states[colMajorIndex(q,found_idx  ,4)],
            states[colMajorIndex(q,found_idx+1,4)],
            states[colMajorIndex(q,found_idx+2,4)]
        };
        interpolated_states[q] = cubic_interp(x_target, xs, ys);
    }
}
...
Real interpolate_dpdx(const Real* xcoords, const Real* Cps, int found_idx, Real x_target, const Oper&oper) {
    ...
    Real h = 1e-4;  // Small step size for derivative approximation TODO: verfiy step is correct (convergence)
...
void interpolate_at_95_both_surfaces(const Real* xcoords, const Real* states, const Real*Cps, const Oper&oper,
    ...
    //  --------------------------top surface BL states interp -----------------------------------------
    int foundIndexTop = find_interp_position(xcoords,0,20,x_target);
    interpolate_on_surface(xcoords, states, foundIndexTop, x_target, topBLStates);
    ...
    // ---------------------------- bottom surface BL interp ------------------------------------------
    int foundIndexBottom = find_interp_position(xcoords,Ncoords-20, Ncoords-1,x_target);
    interpolate_on_surface(xcoords, states,foundIndexBottom, x_target, botBLStates);
    Real dpdxBot = interpolate_dpdx(xcoords,Cps,foundIndexBottom,x_target,oper);
